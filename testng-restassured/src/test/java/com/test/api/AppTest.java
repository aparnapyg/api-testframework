...
public class AppTest {
	...
	/**
	 * Validate that state abbreviations are valid.
	 * @throws JSONException
	 */
	@Test
	public void validateStateAbbreviationsTest() throws JSONException {	
		JSONArray array = null;
		Gson gson = new Gson();
		String prettyPrint = given().baseUri(BASE_URL).log().everything().contentType(ContentType.JSON)
				.when().get(STATES_ENDPOINT).then()
		 ...
		 HashMap<String, String> map = new HashMap<String, String>();
		   ...
		   String REGEX="[A-Z]{2}";
		   for(int i=0; i<array.length(); i++) {
			   System.out.println(Pattern.matches(REGEX,states.get(i).getAbbreviation()));  
			}
	}
